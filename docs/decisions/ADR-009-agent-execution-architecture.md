...
**Revision:** rev 5 (2026-05-13) — incorporates the approved design from [RUSAA-1269](/RUSAA/issues/RUSAA-1269#document-plan) (Slice 1 [RUSAA-1273](/RUSAA/issues/RUSAA-1273) landed as PR #401):
...
3. §7.5 — OAuth 2.0 Authorization Code with PKCE is **superseded for v1** by the SSH-sidecar approach in [RUSAA-1269](/RUSAA/issues/RUSAA-1269#document-plan) §3.2. The `oauth_tokens` table (migration 010, §4.4) stays in the schema — unused by `claude_code` in v1, retained for future provider widening (openai, github, …).
4. §8.3 — new alert `ClaudeCredentialsMissing` (fires when `claude_code` sessions fail with `error_kind="claude_not_logged_in"` at > 10/hour), signalling the operator must re-run `claude /login` via the SSH sidecar.
...
- **External dependency: LiteLLM** — upstream open-source proxy ([`BerriAI/litellm`](https://github.com/BerriAI/litellm)). **v1 operative mode (rev 5):** external instance hosted on **mars**, reached over the dev-stack `rb-net` by `LITELLM_BASE_URL` + virtual key; runtime adapter selects this mode via the three-mode selector documented in [RUSAA-1269](/RUSAA/issues/RUSAA-1269#document-plan) §3.1. **Long-term target (unchanged):** one in-cluster Deployment (default replica = 2 for HA) fronting all non-OAuth model calls. Owns either way: provider abstraction (Anthropic, OpenAI, Bedrock, Vertex, local-Ollama), per-tenant virtual-key issuance, per-tenant budget caps, request/response logging hooks. Rustacean depends on the **LiteLLM HTTP API surface** (OpenAI-compatible Completions / Messages endpoints + virtual-key admin REST). We do NOT fork or vendor LiteLLM source. Wave 7 adds: env-var contract for the external instance today, helm chart values for the in-cluster instance later, virtual-key provisioning script (one key per tenant, scoped by `tenant_id` metadata), and the dial-in for the two LiteLLM-routed runtimes (OpenCode, Pi).
- **`docker/claude-login/`** — SSH sidecar container (rev 5; landed in [RUSAA-1273](/RUSAA/issues/RUSAA-1273) PR #401). Image `rustbrain/claude-login:dev` based on `node:20-bookworm-slim`; exposes `sshd` on container port 22 → host `${CLAUDE_SSH_HOST_PORT:-12222}`; pubkey-only auth (no password, no root), pubkeys supplied via `RB_SSH_AUTHORIZED_KEYS`; non-root user `loginuser` (uid 65532); ships `@anthropic-ai/claude-code` so an operator can SSH in and run `claude /login`. Persists credentials into the named volume `claude-credentials` mounted at `/home/loginuser/.claude`. The agent-runner mounts the same volume **read-only** (`claude-credentials:/home/loginuser/.claude:ro`) and the `claude_code` adapter preflight-checks `${CLAUDE_CONFIG_DIR:-/home/loginuser/.claude}/credentials.json` before spawning a child `claude` (bails with `claude_not_logged_in` if missing — see §8.3 alert). v1 operative path for Claude Max identity; supersedes the OAuth-PKCE flow in §7.5 (which is retained as the multi-tenant future target — see [RUSAA-1269](/RUSAA/issues/RUSAA-1269#document-plan) §3.2).
...
> **Rev 5 amendment (2026-05-13).** v1 ships against an **external LiteLLM** instance hosted on **mars**, not an in-cluster Deployment. The dev-stack `agent-runner` reaches it over `rb-net` by `LITELLM_BASE_URL` + virtual key. The three-mode adapter selector ([RUSAA-1269](/RUSAA/issues/RUSAA-1269#document-plan) §3.1) chooses between `LiteLLM`, `OpenAI-compatible`, and `Direct provider` based on which env vars are set on the agent-runner. This is the operative mode today and the path the OpenCode adapter implements in [RUSAA-1274](/RUSAA/issues/RUSAA-1274). The shared in-cluster Deployment described below remains the **long-term target** — same Decision (Option B), same rationale, deferred only on operational readiness (helm chart, mesh config, secret pipeline) not on architecture. Rev 5 does not reopen Option A vs Option B; it acknowledges that v1's external-mars LiteLLM is operationally a hosted-PaaS variant of Option B (single control surface, central audit) and the migration to in-cluster is a Helm-rollout flip with no architectural surface change.
...
> **Rev 5 amendment (2026-05-13) — OAuth-PKCE for Claude Code superseded for v1.** The OAuth 2.0 Authorization Code with PKCE flow specified below is **superseded for v1** by the SSH-sidecar approach in [RUSAA-1269](/RUSAA/issues/RUSAA-1269#document-plan) §3.2 (landed by [RUSAA-1273](/RUSAA/issues/RUSAA-1273) PR #401). In v1, a human operator SSHes into the `claude-login` sidecar (host port `${CLAUDE_SSH_HOST_PORT:-12222}`, pubkey-only) and runs `claude /login` once; the resulting `~/.claude/credentials.json` is persisted in the `claude-credentials` named volume and read by the agent-runner via the `:ro` mount at `/home/loginuser/.claude` (`CLAUDE_CONFIG_DIR`). The `claude_code` adapter ([`services/agent-runner/src/adapters/claude_code.rs`](../../services/agent-runner/src/adapters/claude_code.rs)) preflight-checks the credentials file and fails fast with `error_kind="claude_not_logged_in"` if missing — see the §8.3 `ClaudeCredentialsMissing` alert. Trade-off accepted: v1 shares one Claude Max identity across the host; per-user / per-tenant Claude identity isolation is deferred. The full OAuth-PKCE spec below remains the **future target** when multi-tenant Claude identity becomes a requirement, and the `control.oauth_tokens` table (migration 010, §4.4) stays in the schema **unused by `claude_code` in v1** — retained for future provider widening (openai, github, …) and for the eventual re-instatement of OAuth-bound Claude. The LiteLLM virtual-key half of this section (below) is unaffected: it governs `opencode` and `pi` runtimes and applies as written in v1, with `LITELLM_BASE_URL` pointing at the external mars instance (§3.4 rev-5 amendment).
...
- `OAuthClaudeRefreshFailureRate` — `rate(agent_oauth_claude_refresh_total{outcome="failure_revoked"}[15m]) / rate(agent_oauth_claude_refresh_total[15m]) > 0.10` for 10m → page Services Engineer. A spike here typically means Anthropic invalidated tokens en masse (provider incident) or our refresh call signature broke after an SDK upgrade. (Inert in v1 while §7.5 OAuth path is superseded by the SSH sidecar; retained for the future re-instatement of the OAuth flow.)
- `ClaudeCredentialsMissing` (rev 5) — `rate(agent_sessions_total{status="failed", runtime_kind="claude_code", error_kind="claude_not_logged_in"}[1h]) > 10` → page Platform Engineer. The agent-runner `claude_code` preflight ([`services/agent-runner/src/adapters/claude_code.rs`](../../services/agent-runner/src/adapters/claude_code.rs)) bails with `claude_not_logged_in` when `${CLAUDE_CONFIG_DIR:-/home/loginuser/.claude}/credentials.json` is absent from the `:ro`-mounted `claude-credentials` volume; > 10/hour means the shared Claude Max identity needs re-authentication via the SSH sidecar (`ssh -p ${CLAUDE_SSH_HOST_PORT:-12222} loginuser@<host>` → `claude /login`). Distinct from `OAuthClaudeRefreshFailureRate` (refresh failure for an existing OAuth row) — `ClaudeCredentialsMissing` indicates **no credential at all**.
