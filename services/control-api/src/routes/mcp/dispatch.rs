...
///
/// Embeds `query` via Ollama, performs ANN search in Qdrant scoped to
/// `tenant_id`, and returns JSON-formatted ranked results.
pub(super) async fn dispatch_search_items(
    ...
    // Validate repo ownership when caller supplies a repo_id filter.
...
/// Execute the `get_item` tool.
///
/// Looks up a code symbol by `(repo_id, fqn)` within the caller's tenant.
pub(super) async fn dispatch_get_item(
    ...
    // Verify repo belongs to this tenant before querying the tenant schema.
    let owned: Option<(Uuid,)> = sqlx::query_as(
        "SELECT id FROM control.repos \
         WHERE id = $1 AND tenant_id = $2 AND archived_at IS NULL",
    )
...
// ---------------------------------------------------------------------------
// Ollama embedding helper (mirrors search.rs — Phase 2 will extract to shared util)
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn get_item_requires_repo_id_and_fqn() {
        ...
        let repo: Option<Uuid> = args
            ...
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse().ok());
        assert!(repo.is_none()); // "not-a-uuid" should not parse
