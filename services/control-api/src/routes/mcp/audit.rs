...
/// Write one audit row for an MCP tool call.
///
/// `outcome` must be `"success"` or `"error"`.
/// Failures are logged at warn level and silently dropped so that an audit
/// write never breaks the caller's response.
