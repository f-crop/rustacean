...
pub fn spawn(pool: Arc<TenantPool>) -> Result<JoinHandle<()>> {
    ...
    let item_cfg = ConsumerCfg::new("projector-pg-items");
    let item_consumer = Consumer::<ParsedItemEvent>::new(&item_cfg)?;
    ...
    let relation_cfg = ConsumerCfg::new("projector-pg-relations");
    let relation_consumer = Consumer::<GraphRelationEvent>::new(&relation_cfg)?;
