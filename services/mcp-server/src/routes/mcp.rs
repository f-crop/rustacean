...
use rb_mcp::{
    ...
    ToolCallParams, ToolsListResult, UNAUTHORIZED_MCP,
};
use uuid::Uuid;
...
async fn dispatch(
    ...
    match rpc.method.as_str() {
        ...
        "tools/list" => handle_tools_list(rpc),
        ...
        if let Ok(p) = serde_json::from_value::<InitializeParams>(params.clone()) {
            ...
            if p.protocol_version != MCP_PROTOCOL_VERSION {
                tracing::warn!(
                    ...
                    "MCP client requested newer protocol"
                );
            }
        }
    }
...
fn handle_tools_list(rpc: JsonRpcRequest) -> McpResult {
...
async fn handle_tools_call(
    ...
    let all_known: Vec<&str> = tools_for_scope("read:items")
        .into_iter()
        .chain(tools_for_scope("read:graph"))
        .collect();

    if !all_known.contains(&params.name.as_str()) {
...
fn require_auth_tenant(auth: &AuthContext) -> Result<Uuid, ()> {
    match auth {
        AuthContext::ApiKey(info)
            if info.scopes.contains(&"read:items".to_owned())
